...
bool translate_cigar_string(string str_cigar, Transcript& trans, unsigned long ul_trans_coord, int i_exon_number, string& str_new_cigar, int& i_end_tlen) {
	...
	int i_extra_soft_clipped = 0;	// if read hangs off of end of reference, keep track of how many bases are soft clipped

	for (int i=0; i<i_num_cigar; i++) {

		if (b_reached_end) {
			if (vec_cigar_codes[i] == 'S' || vec_cigar_codes[i] == 'M' || vec_cigar_codes[i] == 'I') {
				i_extra_soft_clipped += vec_cigar_numbers[i];
			}
			continue;
		}
			...
			case 'D': {
				...
				while (i_cigar_number_remaining > 0) {
					if (ul_cur_pos+i_cigar_number_remaining-1>trans.get_exon_rel_end(i_exon_number)) {
						...
						ul_cur_pos += i_num_M;
						if (i_exon_number+1 >= trans.get_num_exons()) {
							// add remaining 'M' or 'D'
							str_new_cigar += conv2str(i_num_M);
							str_new_cigar += vec_cigar_codes[i];
							i_end_tlen += i_num_M;

							// force a soft clip for the rest
							i_extra_soft_clipped += i_cigar_number_remaining;
	...
	if (i_extra_soft_clipped > 0) {
		str_new_cigar += conv2str(i_extra_soft_clipped);
		str_new_cigar += 'S';
	}
