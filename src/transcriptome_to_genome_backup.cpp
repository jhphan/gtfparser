...
void usage() {
	...
	cout << "\t-ref [reference fasta file headers]" << endl;
...
bool translate_sam_file(map<string,reference_coords>& map_reference_coords, map<string,Transcript>& map_transcript_structure, string str_sam_file) {

	FILE* file_sam;
	string str_cmd = string("java -jar /data/ngs/bin/picard/ViewSam.jar I=")+str_sam_file;
	...
	while (!done) {
		if (read_first_line) {
			...
			if (!translate_sam_record(sr1, map_reference_coords, map_transcript_structure)) {
		...
		if (!read_line(file_sam, ch_buf)) {
			print_sam_record(sr1);	// not paired
			break;
		}
		while (ch_buf[0] == '@') {
			cout << ch_buf << endl;
		...
		if (!translate_sam_record(sr2, map_reference_coords, map_transcript_structure)) {
			cerr << "!error: transcriptome_to_genome: translate_sam_file(): cannot translate sam record" << endl;
			return false;
		}
...
int main(int argc, char* argv[]) {
	...
	map<string,reference_coords> map_reference_coords;
	if (!load_reference_coords(map_reference_coords, str_ref_file)) {
		cerr << "!error: transcriptome_to_genome: cannot load reference coords" << endl;
		return 1;	
	}
	...
	// read sam file
	if (!translate_sam_file(map_reference_coords, map_transcript_structure, str_sam_file)) {
